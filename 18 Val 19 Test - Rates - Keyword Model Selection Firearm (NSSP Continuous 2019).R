...
setwd('X:/Data/NSSP')
data_orig <- read.xlsx('combined_NSSP_weekly_rates.xlsx', sheetIndex = 2, header = TRUE)
...
data <- read.xlsx('combined_NSSP_weekly_rates.xlsx', sheetIndex = 2, header = TRUE)
...
model_backward_lr = lm(Actual.Firearm ~ Overall + Unintentional
                       ,data = train)
...
data2 <- read.xlsx('combined_NSSP_weekly_rates.xlsx', sheetIndex = 2, header = TRUE)
...
model_backward_svr = svm(Actual.Firearm ~ Overall + Unintentional
                         ,data = train2)
...
tuned_svr = tune(svm, Actual.Firearm ~ Overall + Unintentional
...
data3 <- read.xlsx('combined_NSSP_weekly_rates.xlsx', sheetIndex = 2, header = TRUE)
...
model_backward_rf = randomForest(Actual.Firearm ~ Overall + Unintentional
                                 ,data = train3)
